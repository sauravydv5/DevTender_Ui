...
export default function PaymentPage() {
  ...
  return (
    <div className="min-h-screen px-6 py-16 bg-gray-100 dark:bg-gray-900 md:px-20">
      <ToastContainer />
      <h1 className="mb-6 text-5xl font-extrabold text-center text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 via-purple-500 to-pink-500 animate-pulse">
        Upgrade to Premium 🚀
      </h1>
      ...
      <div className="grid max-w-6xl gap-10 mx-auto md:grid-cols-3">
        {plans.map((plan, i) => (
          <div
            key={i}
            className={`relative p-8 border-4 rounded-3xl shadow-xl ${
              plan.color
            } ${
              plan.highlighted
                ? "scale-105 animate-bounce ring-4 ring-yellow-300"
          ...
          >
            ...
            <button
              disabled={loadingPlan === plan.name}
              onClick={() => handlePlanSelect(plan.name)}
              className={`mt-10 w-full px-8 py-4 font-semibold text-white bg-indigo-600 shadow-md rounded-xl hover:bg-indigo-700 ${
                loadingPlan === plan.name ? "cursor-wait opacity-80" : ""
              }`}
            >
              {loadingPlan === plan.name
